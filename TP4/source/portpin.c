...
void PINloadISR(uint8_t i, void (*func)(void), uint8_t pin) {
...
void PORTB_IRQHandler(void) {
	...
	for (i = 0; i < N_ISRPINS; i++) {
		if (PIN2PORT(isrpin[i].pin) == PORT_B) {
			if(port[PORT_B]->PCR[PIN2NUM(isrpin[i].pin)] & PORT_PCR_ISF_MASK) { //nos fijamos si es el pin correspondiente a la interrupcion
				port[PORT_B]->PCR[PIN2NUM(isrpin[i].pin)] |= PORT_PCR_ISF_MASK; //ponemos en cero el interrupt flag. w1c.
				isrpin[i].funcptr();
			}
		}
	}
...
void PORTD_IRQHandler(void) {
	...
	for (i = 0; i < N_ISRPINS; i++) {
		if (PIN2PORT(isrpin[i].pin) == PORT_D) {
			if(port[PORT_D]->PCR[PIN2NUM(isrpin[i].pin)] & PORT_PCR_ISF_MASK) { //nos fijamos si es el pin correspondiente a la interrupcion
				port[PORT_D]->PCR[PIN2NUM(isrpin[i].pin)] |= PORT_PCR_ISF_MASK; //ponemos en cero el interrupt flag. w1c.
				isrpin[i].funcptr();
			}
		}
	}
